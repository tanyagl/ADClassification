...
for i=1:length(input_roi)
    max_sizeX = max(size(input_roi(i).prj.p12{1},1),max_sizeX);
    max_sizeY = max(size(input_roi(i).prj.p12{1},2),max_sizeY);
end
if ~mod(max_sizeX,2)
...
for i=1:length(all_sbj_rois)
    ...
    switch input_roi(i).label.diagnosis
        case 0
            ...
            updated_roi(i).features = [CalculateMoment(cur_prj,1,1);CalculateMoment(cur_prj,1,2);CalculateMoment(cur_prj,2,1);CalculateMoment(cur_prj,2,2);entropy(cur_prj)];
