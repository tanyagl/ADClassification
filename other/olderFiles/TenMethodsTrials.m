...
% 
% 
randInd = randperm(length(TrainingData))';
TrainingData = TrainingData(randInd,:);
TrainingLabels = TrainingLabels(randInd,:);
randInd = randperm(length(TestingData))';
TestingData = TestingData(randInd,:);
TestingLabels = TestingLabels(randInd,:);

CNADTrainingData = TrainingData(TrainingLabels==0 | TrainingLabels==1,:);
CNADTrainingLabels = TrainingLabels(TrainingLabels==0 | TrainingLabels==1,:);
CNADTestingData = TestingData(TestingLabels==0 | TestingLabels==1,:);
CNADTestingLabels = TestingLabels(TestingLabels==0 | TestingLabels==1,:);
% 
% 
%  model = svmtrain_liblinear(CNADTrainingLabels, sparse(CNADTrainingData));
%          
% [predict_label, ac, prob_values] = svmpredict_liblinear(CNADTestingLabels, sparse(CNADTestingData), model, '-b 1'); % run the SVM model on the test data
% 
% 
   ...
   %    [C,S] = meshgrid(-5:2:15, 0:6);
    ...
    for i=1:numel(C)
        cv_acc(i) = svmtrain_libsvm(CNADTrainingLabels, CNADTrainingData, sprintf('-t 2 -c %f -g %f -v %d', 2^C(i), 2^gamma(i), folds));
      %  cv_acc(i) = svmtrain_liblinear(CNADTrainingLabels, sparse(CNADTrainingData), sprintf('-c %f -v %d', 2^C(i), folds));
    end
...
% best_gamma = 2^gamma(idx);
    %# ...
   model = svmtrain_libsvm(CNADTrainingLabels, CNADTrainingData, sprintf('-t 2 -c %f -g %f ', best_C, best_gamma));
   model = svmtrain_liblinear(CNADTrainingLabels, sparse(CNADTrainingData), sprintf('-c %f', best_C));
    
 %[predict_label, ac, ~] = svmpredict_libsvm(CNADTestingLabels, CNADTestingData, model); % run the SVM model on the test data
[predict_label,~,p] = svmpredict_liblinear(CNADTestingLabels, sparse(CNADTestingData), model);
...
% num_subjects = length(AllTestingData{1});
% 
% for i=1:num_subjects
%     for j=1:8
%     AllTestingData{j}(i).IsTraining = 0;
%     end
% end
% 
